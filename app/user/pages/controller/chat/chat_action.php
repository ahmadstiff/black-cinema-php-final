...
if ($action == 'send' && $_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($user_role == 'user') {
        ...
        $stmt = $conn->prepare("SELECT chat.*, user.user_username FROM chat JOIN user ON chat.user_id = user.user_id WHERE (chat.user_id = ? AND chat.chat_with = 1) OR (chat.user_id = 2 AND chat.chat_with = ?) ORDER BY chat.timestamp ASC");
        $stmt->bind_param("ii", $user_id, $chat_with);
    }
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo '<strong>' . htmlspecialchars($row['user_username']) . ':</strong> ' . htmlspecialchars($row['message']);
        echo '<span class="block text-xs text-gray-300">' . $timeFormatted . '</span>';
        echo '</div>';
    }
