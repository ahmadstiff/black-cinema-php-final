                ...
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    <thead class="text-xs text-black/20 uppercase bg-gray-50 dark:bg-black/20 dark:text-gray-400">
                        <tr>
                            ...
                            <th scope="col" class="px-4 py-3">Resolusi Layar</th>
                    ...
                    <tbody id="paymentList">
                        ...
                        ?>
                            <tr class="border-b dark:border-black/20">
                                ...
                                <td class="px-4 py-3">
                                    <a href="edit_payment_plan?id=<?= $row['id']; ?>" class="text-green-600 hover:text-green-800">
                                        <i class="fa-solid fa-pen-to-square text-green-500"></i>
                                    </a>
                                </td>
                                <td class="px-4 py-3">
                                    <button class="text-red-600 hover:text-red-800" onclick="deletePaymentPlan(<?= $row['id']; ?>)"><i class="fa-solid fa-trash text-red-500"></i></button>
                                </td>
                            </tr>
                        <?php } ?>
                    </tbody>
...
<div id="add-modal" tabindex="-1" aria-hidden="true" class="hidden fixed z-50 inset-0 flex items-center justify-center backdrop-blur-sm">
    <div class="rounded-lg shadow-lg p-6 overflow-y-auto max-w-md">
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <form id="addPaymentPlanForm" class="p-4 md:p-5" action="pages/controller/payments/payment_plan/add_payment_plan.php" method="POST">
                <input type="hidden" id="paymentPlanId" name="paymentPlanId" value="">
                ...
                <div class="flex flex-row w-full gap-3">
                    <div class="mb-4 w-1/2">
                        ...
                        <input type="number" id="screenResolution" name="screenResolution" placeholder="0" required class="w-full px-3 py-2 placeholder-gray-300 border border-gray-300 rounded-md focus:outline-none focus:ring-gray-500 focus:border-gray-500 dark:bg-gray-800 dark:border-gray-600 dark:focus:ring-gray-500 dark:focus:border-gray-500 dark:text-white">
                    </div>
                    <div class="mb-4 w-1/2">
                ...
                <div class="mt-2 flex justify-center w-full">
                    <button type="submit" id="savePaymentPlanBtn" class="bg-gray-600 hover:bg-gray-700 text-white rounded-md px-4 py-2 transition duration-200 focus:outline-none focus:ring-4 focus:ring-gray-300">Tambah</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const addPaymentPlanForm = document.getElementById('addPaymentPlanForm');
        ...
        addPaymentPlanForm.addEventListener('submit', function(event) {
            event.preventDefault();

            const formData = new FormData(addPaymentPlanForm);

            fetch('pages/controller/payments/payment_plan/add_payment_plan.php', {
                    method: 'POST',
                    body: formData
                })
...
<script>
    function deletePaymentPlan(paymentPlanId) {
        Swal.fire({
            title: 'Are you sure?',
            ...
            if (result.isConfirmed) {
                fetch(`pages/controller/payments/payment_plan/delete_payment_plan.php?id=${paymentPlanId}`, {
                        method: 'DELETE'
                    })
                    .then(response => response.json())
                    .then(data => {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        editPaymentPlanBtns.forEach(function(btn) {
            btn.addEventListener('click', function(e) {
                e.preventDefault();
                const paymentPlanId = btn.getAttribute('data-id');
